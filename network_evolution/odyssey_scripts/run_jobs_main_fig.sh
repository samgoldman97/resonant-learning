...
python3 ~/network_evolution/main.py --timestamp  "${SLURM_ARRAY_TASK_ID}${SLURM_ARRAY_JOB_ID}" --n 500 \
                        ...
                        --control_param 1.9 --simulation_type $SIMTYPE \
                        --target_lengths `seq 2 2 $LONGPERIOD` --target_node -1 --periods `seq 2 2 $LONGPERIOD` \
                        --resonance_period -1 --prob_perturb 0 --constant_hub $CONSTHUB --generations 10 \
                        --switch_time -1 
