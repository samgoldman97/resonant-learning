...
class AttractorGraph:
    ...
    def pretty_print(self):
        ...
        for per in pers:
            ...
            num_basin = list(
                map(lambda x: len(x), [j for j, _ in self.per_map[per].values()])
            )
            print("Number of nodes in each basin: ", num_basin)
            print("-----------------")
    ...
    def explore_state_block(self, block_nodes, block_state, t=2000):
        ...
        for index, row in enumerate(tbl[: stop + 1]):
            ...
            if self._was_explored(state=row_str, period=AttractorGraph.NO_OSCIL):
                break
            else:
                self._add_edge(
    ...
    def explore_state_free(self, t=2000):
        ...
        for index, row in enumerate(tbl[: stop + 1]):
            row_str = row.tostring()
            if self._was_explored(state=row_str, period=AttractorGraph.NO_BLOCK):
                break
            else:
                self._add_edge(
    ...
    def explore_oscil(self, per, t=2000, explore_null=True, oscil_shift=0):
        ...
        for index, row in enumerate(tbl[: stop + 1]):
            ...
            # If we've already seen this node with no phase!
            if self._was_explored_in_attr(
                state=row_str, period=per, attr=oscil_attr, no_phase=no_phase
            ):
    ...
    def depth_to_old_attr(self, state, t=2000):
        ...
        if state not in self._get_all_nodes(period=AttractorGraph.NO_OSCIL):
        ...
        for attr in self.per_map[AttractorGraph.NO_OSCIL]:
            nodes = self.per_map[AttractorGraph.NO_OSCIL][attr][0]
            edges = self.per_map[AttractorGraph.NO_OSCIL][attr][1]
            if state in nodes:
                # If this is the right attr... preform depth first search
                cur_node = state
                depth = 0
                while cur_node not in attr:
                    next_node = list(edges[cur_node])[0]
                    cur_node = next_node
                    depth += 1
                return depth
    ...
    def phase_explore(self, per, sample_num=None, t=2000):
        ...
        nodes = self._get_all_nodes(period=per, datum=True)
        phase_shifted_nodes = [k for k, v in nodes.items() if not v]
    ...
    def phase_explore_attr(self, per, attr, sample_num=None, t=2000):
        ...
        """
        # get all the nodes with data
        nodes = self._get_all_nodes_attr(period=per, attr=attr, datum=True)
        phase_shifted_nodes = [k for k, v in nodes.items() if not v]
    ...
    def phase_explore_attr_cycle(self, per, attr, sample_num=None, t=2000):
        ...
        """
        nodes = self._get_all_nodes_attr(period=per, attr=attr, datum=True)
        # We make sure k is in the attractor...
        phase_shifted_nodes = [k for k, v in nodes.items() if not v and k in attr]
    ...
    def explore_new_period(
        ...
        already_explored = self._get_all_nodes(period=per, datum=True)

        # Check that we haven't explored the node already
        unexplored_nodes = [
            k for k in nodes if k not in already_explored or not already_explored[k]
        ]
    ...
    def cross_compare_periods(
        ...
        def get_all_nodes(per):
            return {
                node
                for node, no_phase in self._get_all_nodes(per, datum=True).items()
                if no_phase
            }
    ...
    def check_parsimony(self, test_per, target_per, zero_phase=True):
        ...
        TODO: Consider enforcing the uniqueness constraint
        ...
        per1_nodes = set(
            [
                i
                for i, j in self._get_all_nodes(period=test_per, datum=True).items()
                if j
            ]
        )
        per2_nodes = set(
            [
                i
                for i, j in self._get_all_nodes(period=target_per, datum=True).items()
                if j
            ]
        )
    ...
    def check_attr_parsimony_ctrl(
        ...
        per2_nodes_explored = set(
            [
                i
                for i, j in attr_graph2._get_all_nodes(
                    period=target_per, datum=True
                ).items()
                if j
            ]
        )
    ...
    def export_bipartite_multigraph(cls, graph1, graph2, per1, per2, outfile):
        ...
        per1_mapping = graph1._map_nodes_attr(per1)
        per2_mapping = graph2._map_nodes_attr(per2)
    ...
    def export_graph(self, outfile):
        ...
        ### Get all attractors...
        # Label each node so that we can always color by attractor!
        all_attrs = [
            attr
            for per, attr_dict in self.per_map.items()
            for attr in attr_dict.keys()
            if per != AttractorGraph.NO_BLOCK and per != AttractorGraph.NO_OSCIL
        ]
        alt_attr_names = {attr: f"Attr_{index}" for index, attr in enumerate(all_attrs)}
        for attr in all_attrs:
            name = all_attrs
            for node in attr:
                if "attr_name" in g.node[node]:
                    g.node[node]["attr_name"] = (
                        str(g.node[node]["attr_name"]) + f"_{name}"
                    )
                else:
                    g.node[node]["attr_name"] = str(f"{name}")
                    ## Todo: change this to add one more setting so we can distinguish between control attrs and new attrs
                    g.node[node]["oscil_attractor"] = 1

        # If it's an oscil, give it a 2...
        all_attrs_ = [attr for attr in self.per_map[AttractorGraph.NO_OSCIL]]
        for attr in all_attrs_:
            name = all_attrs
            for node in attr:
                g.node[node]["oscil_attractor"] = 2
        ...
        for index, attractor in enumerate(self.per_map[AttractorGraph.NO_OSCIL].keys()):
        ...
        for node, a in g.nodes(data=True):
            ...
            # If not a special node..
            if "oscil_attractor" not in a:
                g.node[node]["oscil_attractor"] = 0
    ...
    def get_all_attr_states(self, period):
        ...
        period = str(period)
        return set([node for i in self.per_map[period].keys() for node in i])
    ...
    def _get_all_nodes(self, period, datum=False, warning=False):
        """
        Get all the nodes associated with a given period
        """
        # Add edges to graph from the control
        ...
        for node_dict, _ in self.per_map[period].values():
            for node in node_dict.keys():
                if node not in cumulative_dict:
                    cumulative_dict[node] = node_dict[node]
                else:
                    # Check that one of these had no phase shift if this is in multiple attractor!
                    if warning:
                        print("Warning: Node found in multiple attractors..")
                    cumulative_dict[node] = cumulative_dict[node] or node_dict[node]

        if datum:
            return cumulative_dict
        else:
            return set(cumulative_dict.keys())

    def _get_all_nodes_attr(self, period, attr, datum=False):
        """
        Get all nodes in a given attractor associated with this period
        """
        ...
        node_dict, _ = self.per_map[period][attr]

        if datum:
            return node_dict
        else:
            return set(node_dict.keys())
    ...
    def _get_all_edges(self, period):
        ...
        return {
            edge
            for _, edge_dict in self.per_map[period].values()
            for edge in self._get_edges(edge_dict)
        }

    def _get_edges(self, edge_dict):
        """
        Converts a dict of one to many pairings to a list of one to one pairings
        """
        return {(k, val) for k, vals in edge_dict.items() for val in vals}

    def _map_nodes_attr(self, per):
        """
        Create a mapping between nodes and their corresponding attractor
        ...
        if per not in self.per_map:
            return dict()
        r = dict()
        for attr, (node_dict, edge_dict) in self.per_map[per].items():
            for node in node_dict.keys():
                ...
                if node in r:
                    print("This node is in multiple attractors!")
                    r[node].append(attr)
                else:
                    r[node] = [attr]
        return r

    def _add_edge(self, period, s1, s2, attr, was_start=False):
        """
        ...
        Args:
                ...
                was_start: Bool to denote if this state should be marked as the first one
        """
        ...
        if attr not in self.per_map[period]:
            self.per_map[period][attr] = (
                dict(),
                dict(),
            )  # ({s1: was_start, s2: False}, {s1 : set(s2)} )
        # else:
        # Add to list of nodes
        # Note: in the future we might want to distinguish w/o overwriting these start runs
        if s1 in self.per_map[period][attr][0]:
            self.per_map[period][attr][0][s1] = (
                was_start or self.per_map[period][attr][0][s1]
            )
        else:
            self.per_map[period][attr][0][s1] = was_start
        # Now adjust edges
        if s1 not in self.per_map[period][attr][1]:
            self.per_map[period][attr][1][s1] = set([s2])
        else:
            self.per_map[period][attr][1][s1].add(s2)
    ...
    def _was_explored_in_attr(self, state, period, attr, no_phase):
        ...
        if not no_phase:
            return False
        elif period not in self.per_map:
            return False
        elif attr not in self.per_map[period]:
            return False
        elif state not in self.per_map[period][attr][0]:
            return False
        else:
            # Return true if we've already seen this node when there's no phase!
            return self.per_map[period][attr][0][state]
